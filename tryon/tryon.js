...
(() => {
  ...
  // Android использует Capacitor-плагин, браузер — тот же ONNX-файл через
  // адаптер из nail-seg-plugin.js. Оба пути возвращают одну и ту же PNG-маску.
  const NailSeg = window.Capacitor?.isNativePlatform?.()
    ? window.Capacitor.Plugins?.NailSegmentation
    : window.ManiNailSegmentation;
  ...
  function checkNativePlugin(){
    ...
    if (!NailSeg) {
      setStatus(ui.model, 'Распознавание пока недоступно. Обновите страницу и попробуйте снова.', 'error');
      return false;
    }
